...
- (id)initWithFrame:(CGRect)frame image:(NSArray *)imagesData animationDuration:(NSTimeInterval)animationDuration
{
    ...
    [self loadImage:imagesData titltData:nil];
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.pageControl.center = CGPointMake(self.scrollView.right- 40, self.scrollView.bottom-15);
...
- (void)loadImage:(NSArray *)imagesData titltData:(NSArray *)titleData
{
    ...
    for (int i = 0; i < [imagesData count]; i++) {
        ...
        if (titleData && titleData.count>0) {
            UIView * bottomBgView = [[UIView alloc] initWithFrame:CGRectMake(0, self.bounds.size.height-35, Screen_Width, 35)];
            bottomBgView.backgroundColor = [UIColor blackColor];
            bottomBgView.alpha = 0.5;
            [imageView addSubview:bottomBgView];
            
            UILabel *titlelLable = [[UILabel alloc] initWithFrame:CGRectMake(15,self.bounds.size.height-35 , Screen_Width-15, 35)];
            titlelLable.textColor = [UIColor whiteColor];
            titlelLable.font = [UIFont boldSystemFontOfSize:14];
            titlelLable.text = titleData[i];
            [imageView addSubview:titlelLable];
        }
